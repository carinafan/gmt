...
for (i in 1:n) {
  ...
  # pull repetitions
  temp_reps = raw_user_data %>% 
    filter(tag2 == "Double Word Stated") %>% 
    nrow()
    
  temp_reps_correct = raw_user_data %>% 
    filter(tag2 == "Double Word Reacted To") %>% 
    nrow()
    
  temp_ups = raw_user_data %>% 
    filter(tag2 == "Up Arrow Pressed Wrong Time") %>% 
    nrow()
  ...
  df$repetitions_heard[i] = temp_reps
  df$repetitions_correct[i] = temp_reps_correct
  df$ups_incorrect[i] = temp_ups
...
for (i in 1:nrow(dict)) {
  switch(dict$column_label[i],
         ...
         "stops" = {
           dict$description[i] = "number of stops enacted"
           dict$type[i] = "integer"
           dict$value_range[i] = "0 to 6 ish"
         },
         
         "repetitions_heard" = {
           dict$description[i] = "number of repeated words presented"
           dict$type[i] = "integer"
           dict$value_range[i] = "0 to 5"
         },
         
         "repetitions_correct" = {
           dict$description[i] = "number of repetitions responded to correctly"
           dict$type[i] = "integer"
           dict$value_range[i] = "0 to 5"
         },
         
         "ups_incorrect" = {
           dict$description[i] = "number of times up arrow was hit incorrectly (i.e., not in response to a repetition)"
           dict$type[i] = "integer"
           dict$value_range[i] = "0 or higher"
         }
